...
router.post('/request', async (req: Request, res: Response) => {
  ...
  const summaryReq = { ...req, query: d.partyType === 'restaurant' ? { restaurantId: partyId } : {} } as Request;
  const roles = req.user?.roles || [];
  let available = 0;
  ...
  const result = await pool.query(`INSERT INTO payouts (party_type, party_id, bank_account_id, amount, reference, status) VALUES ($1, $2, $3, $4, $5, 'pending') RETURNING *`, [d.partyType, partyId, d.bankAccountId ?? null, d.amount, d.reference ?? null]);
