...
type LoyaltyPayload = { loyalty?: { loyalty_points: number } };
...
export function ProfilePage() {
  ...
  const payoutRole = user?.roles.includes('driver') ? 'driver' : (user?.roles.includes('restaurant_owner') || user?.roles.includes('restaurant_manager')) ? 'restaurant' : '';
  const [bankForm, setBankForm] = useState({ holderScope: payoutRole || 'driver', accountName: '', bankName: '', accountNumber: '', branchCode: '', accountType: '' });
  ...
  useEffect(() => {
    ...
    if (payoutRole) api<PayoutSummary>('/payouts/summary', {}, token).then(setSummary).catch(() => undefined);
    if (payoutRole) api<BankAccount[]>('/payouts/bank-accounts', {}, token).then(setBankAccounts).catch(() => undefined);
    if (user?.roles.includes('customer')) api<LoyaltyPayload>('/customer/home', {}, token).then((data) => setLoyaltyPoints(Number(data.loyalty?.loyalty_points || 0))).catch(() => undefined);
  }, [token, payoutRole, user?.roles]);
  ...
  return <div className="page-shell">
    <div className="page-header"><div><div className="eyebrow">Profile</div><h1>Welcome, {user?.fullName || 'there'}</h1><p>Update your details and keep loyalty points in your profile. Payout tools only appear for drivers and restaurants.</p></div></div>
    ...
    <div className="grid-two" style={{ marginTop: 18 }}>
      <section className="panel">
        <div className="panel-header"><h3>Loyalty points</h3></div>
        <div className="feature-item">Available loyalty points: <strong>{loyaltyPoints}</strong></div>
      </section>
      {payoutRole ? <section className="panel">
