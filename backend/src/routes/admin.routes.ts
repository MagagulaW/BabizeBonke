...
router.get('/payouts', async (_req: Request, res: Response) => {
  const [rows, totals] = await Promise.all([
    pool.query(`
      ...
      FROM payouts p
      LEFT JOIN users u ON p.party_type = 'driver' AND u.id = p.party_id
      LEFT JOIN restaurants r ON p.party_type = 'restaurant' AND r.id = p.party_id
      LEFT JOIN bank_accounts ba ON ba.id = p.bank_account_id
      LEFT JOIN users approver ON approver.id = p.approved_by_user_id
      ORDER BY CASE p.status WHEN 'pending' THEN 0 WHEN 'approved' THEN 1 WHEN 'processing' THEN 2 WHEN 'paid' THEN 3 ELSE 4 END, p.created_at DESC
      LIMIT 200
    `),
    pool.query(`
      SELECT
        COALESCE(SUM(CASE WHEN o.status = 'delivered' THEN o.total_amount ELSE 0 END),0)::numeric(12,2) AS gross_collected,
        COALESCE(SUM(CASE WHEN o.status = 'delivered' THEN o.total_amount * (LEAST(GREATEST(r.commission_rate, 10), 15) / 100.0) ELSE 0 END),0)::numeric(12,2) AS system_commission_retained,
        COALESCE(SUM(CASE WHEN d.status = 'delivered' THEN d.driver_payout_estimate ELSE 0 END),0)::numeric(12,2) AS driver_earned_total,
        COALESCE(SUM(CASE WHEN o.status = 'delivered' THEN o.total_amount * ((100 - LEAST(GREATEST(r.commission_rate, 10), 15)) / 100.0) ELSE 0 END),0)::numeric(12,2) AS restaurant_earned_total,
        COALESCE((SELECT SUM(amount) FROM payouts WHERE status IN ('pending','approved','processing')),0)::numeric(12,2) AS pending_claims_total,
        COALESCE((SELECT SUM(amount) FROM payouts WHERE status = 'paid'),0)::numeric(12,2) AS paid_claims_total
      FROM orders o
      JOIN restaurants r ON r.id = o.restaurant_id
      LEFT JOIN deliveries d ON d.order_id = o.id
    `)
  ]);
  return ok(res, { totals: totals.rows[0], claims: rows.rows });
});

router.get('/payout-balances', async (_req: Request, res: Response) => {
  const [drivers, restaurants] = await Promise.all([
    pool.query(`
      SELECT u.id, u.full_name, u.email,
             COALESCE(SUM(d.driver_payout_estimate),0)::numeric(12,2) AS total_earned,
             COALESCE((SELECT SUM(amount) FROM payouts p WHERE p.party_type = 'driver' AND p.party_id = u.id AND p.status = 'paid'),0)::numeric(12,2) AS total_claimed,
             COALESCE((SELECT SUM(amount) FROM payouts p WHERE p.party_type = 'driver' AND p.party_id = u.id AND p.status IN ('pending','approved','processing')),0)::numeric(12,2) AS pending_claims,
             (COALESCE(SUM(d.driver_payout_estimate),0) - COALESCE((SELECT SUM(amount) FROM payouts p WHERE p.party_type = 'driver' AND p.party_id = u.id AND p.status IN ('pending','approved','processing','paid')),0))::numeric(12,2) AS available_balance
      FROM users u
      JOIN user_roles ur ON ur.user_id = u.id AND ur.role_code = 'driver'
      LEFT JOIN deliveries d ON d.current_driver_user_id = u.id AND d.status = 'delivered'
      GROUP BY u.id
      ORDER BY u.full_name
    `),
    pool.query(`
      SELECT r.id, r.display_name, r.support_email,
             COALESCE(SUM(CASE WHEN o.status = 'delivered' THEN o.total_amount * ((100 - LEAST(GREATEST(r.commission_rate, 10), 15)) / 100.0) ELSE 0 END),0)::numeric(12,2) AS total_earned,
             COALESCE((SELECT SUM(amount) FROM payouts p WHERE p.party_type = 'restaurant' AND p.party_id = r.id AND p.status = 'paid'),0)::numeric(12,2) AS total_claimed,
             COALESCE((SELECT SUM(amount) FROM payouts p WHERE p.party_type = 'restaurant' AND p.party_id = r.id AND p.status IN ('pending','approved','processing')),0)::numeric(12,2) AS pending_claims,
             (COALESCE(SUM(CASE WHEN o.status = 'delivered' THEN o.total_amount * ((100 - LEAST(GREATEST(r.commission_rate, 10), 15)) / 100.0) ELSE 0 END),0) - COALESCE((SELECT SUM(amount) FROM payouts p WHERE p.party_type = 'restaurant' AND p.party_id = r.id AND p.status IN ('pending','approved','processing','paid')),0))::numeric(12,2) AS available_balance
      FROM restaurants r
      LEFT JOIN orders o ON o.restaurant_id = r.id
      GROUP BY r.id
  ...
  return ok(res, { drivers: drivers.rows, restaurants: restaurants.rows });
});

router.patch('/payouts/:id/status', async (req: Request, res: Response) => {
  const status = String(req.body?.status || '');
  const allowed = ['approved','processing','paid','failed','reversed','pending'];
  if (!allowed.includes(status)) return fail(res, 400, 'Invalid payout status');
  const result = await pool.query(`
    UPDATE payouts
       SET status = $2::payout_status,
           approved_by_user_id = CASE WHEN $2 IN ('approved','processing','paid') THEN $3 ELSE approved_by_user_id END,
           paid_at = CASE WHEN $2 = 'paid' THEN now() ELSE paid_at END,
           updated_at = now()
     WHERE id = $1
     RETURNING *
  `, [req.params.id, status, req.user?.userId || null]);
  if (!result.rowCount) return fail(res, 404, 'Payout request not found');
  return ok(res, result.rows[0], `Payout marked ${status}`);
});
