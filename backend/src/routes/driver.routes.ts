...
router.get('/deliveries', async (req: Request, res: Response) => {
  ...
  const result = await pool.query(`
    SELECT d.id, d.order_id, d.status, d.pickup_eta_mins, d.dropoff_eta_mins, d.driver_payout_estimate,
           ...
           a.label AS address_label, a.address_line1, a.address_line2, a.suburb, a.city, a.province, a.postal_code, a.delivery_instructions,
    ...
    WHERE d.current_driver_user_id = $1
       OR (
         d.current_driver_user_id IS NULL
         AND d.status IN ('awaiting_dispatch','offer_in_progress','assigned')
         AND COALESCE(o.status, 'placed') NOT IN ('cancelled','delivered')
       )
    ORDER BY CASE WHEN d.current_driver_user_id = $1 THEN 0 ELSE 1 END, d.created_at DESC
  `, [userId]);
...
router.post('/deliveries/:deliveryId/accept', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!['awaiting_dispatch','offer_in_progress','assigned','accepted'].includes(delivery.status)) throw new Error('Delivery is no longer available');
    ...
    const claim = await client.query(
      `UPDATE deliveries
       ...
       WHERE id = $1
         ...
         AND status IN ('awaiting_dispatch','offer_in_progress','assigned','accepted')
