...
function assertInternationalPhone(value: string, fieldName = 'Phone number') {
    if (!/^\+[1-9]\d{7,14}$/.test(value)) {
        throw new Error(`${fieldName} must include a valid country code, for example +27821234567`);
    }
}
async function assertUserIdentityAvailable(client: import("pg").PoolClient, email: string, phone?: string | null) {
    const checks = phone
...
async function buildSessionForUser(userId: string) {
