...
router.patch('/:restaurantId/orders/:orderId/status', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (status === 'cancelled') {
      ...
      await client.query(`UPDATE deliveries SET status = CASE WHEN status IN ('awaiting_dispatch','offer_in_progress','assigned') AND current_driver_user_id IS NULL THEN 'offer_in_progress' ELSE status END, updated_at = now() WHERE order_id = $1 AND status NOT IN ('cancelled','delivered','failed')`, [orderId]);
      notes = 'Restaurant accepted the order and opened it for driver dispatch';
    ...
    emitToRestaurant(restaurantId, 'order:status_changed', { orderId, status, source: 'restaurant', reason: reason ?? null, estimatedPrepMins: estimatedPrepMins ?? null });
    if (status === 'confirmed' || status === 'ready_for_pickup') {
      emitToRestaurant(restaurantId, 'delivery:dispatch_ready', { orderId, status });
    }
